...
query_result *btree_query_1_svc(btree_query_args args, struct svc_req *req)
{
    ...
    query_result *res = get_vector(local_vecs[0]);
    u_int64_t *result_vector = res->vector.vector_val;
    unsigned int result_vector_len = res->vector.vector_len;
    int i;
    ...
    for (i = 0; i < sub_len; i++) {
        ...
        CLIENT *clnt = clnt_create(a.this_machine_address, BTREE_QUERY_PIPE, BTREE_QUERY_PIPE_V1, "tcp");
        if (clnt == NULL) {
    ...
    for (i = 0; i < sub_len; i++) {
        char op = args.subquery_ops.subquery_ops_val[i - 1];
